...
function refresh() { if (!game) return; const unknown=game.network.validPairs().some(([pre,post])=>(game.guesses.get(pairKey(pre,post))||TYPES.UNKNOWN)===TYPES.UNKNOWN);ui.setPair(game.selection,game.locked);ui.setWatchedNeuron(game.watchedNeuron);ui.updateProgress(game.network,game.guesses);ui.renderMatrix(game.network,game.guesses,game.selection,game.locked,pickPair);ui.el.lock_button.disabled=game.locked;ui.el.mark_unknown_unconnected_button.disabled=game.locked||!unknown;ui.el.mode_label.textContent=game.locked?'Results: true network shown':game.selection.shiftMode?'Pair selection: shift-click a neuron':'Playing: Ctrl-click to stimulate'; }
...
function markUnknownAsUnconnected() { if (game.locked) return; let updated=0; for (const [pre,post] of game.network.validPairs()) { const key=pairKey(pre,post); if ((game.guesses.get(key)||TYPES.UNKNOWN)===TYPES.UNKNOWN) { game.guesses.set(key,TYPES.NONE); updated+=1; } } refresh(); ui.announce(`${updated} unknown pair${updated===1?'':'s'} marked unconnected.`); }
...
ui.el.mark_unknown_unconnected_button.addEventListener('click',markUnknownAsUnconnected);
